...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getRiskLimits() {
    const data = await prisma.risk_limit
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'risk_limit'));
    return res.status(200).json(data);
  }

  async function createRiskLimit() {
    await riskLimitValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.risk_limit.create({
      data: body,
    });
    return res.status(200).json(data);
  }
